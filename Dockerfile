...
RUN apk add gcc g++
RUN go env && CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build -a -ldflags '-linkmode external -extldflags "-static"' -o next-terminal main.go

FROM linuxserver/guacd:latest
